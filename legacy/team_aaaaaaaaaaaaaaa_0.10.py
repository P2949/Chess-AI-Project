...
def _square_for_color(color, square):
...
def _development_score(board, color, phase):
    ...
    for sq in board.pieces(chess.ROOK, color):
        if sq not in STARTING_SQUARES[color][chess.ROOK] and phase > 0.65:
            score -= 18.0 + 18.0 * phase

    for sq in board.pieces(chess.QUEEN, color):
        if phase > 0.75:
            score -= 8.0 * max(0.0, 4.5 - _center_distance(sq))
...
def _king_safety(board, color, phase):
    ...
    direct_checkers = len(board.attackers(enemy, king_sq))
    bonus -= (10.0 + 16.0 * phase) * ring_attacks
    bonus -= (14.0 + 20.0 * phase) * direct_checkers
...
def _material_and_positional(board, phase):
    ...
    for sq, piece in board.piece_map().items():
        ...
        idx = _square_for_color(color, sq)
