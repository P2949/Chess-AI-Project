...
def _contempt_score() -> float:
    return CONTEMPT if _ROOT_COLOR == chess.BLACK else -CONTEMPT
...
def _pawn_storm(board: chess.Board, eg: float) -> float:
    ...
    for color in (chess.WHITE, chess.BLACK):
        ...
        for sq in board.pieces(chess.PAWN, color):
            ...
            if advancement >= 4:
                score += sign * (advancement - 3) * 12 * mg_weight
    return score
    ...
    for sq in chess.SQUARES:
        ...
        if board.is_attacked_by(enemy, sq):
            ...
            if defenders < attackers:
                val  = PIECE_VALUES[piece.piece_type] * 0.35
                sign = -1 if piece.color == chess.WHITE else 1
                score += sign * val
    return score
    ...
    for sq in chess.SQUARES:
        ...
        for tsq in chess.SquareSet(chess.BB_KNIGHT_ATTACKS[sq]):
            ...
            if len(future_vals) >= 2:
                ...
                weight = 0.07 if occupant is None else 0.035
...
def _king_proximity_mop_up(board: chess.Board, eg: float, material_score: float) -> float:
    ...
    if material_score > 200:
        ...
        for q_sq in board.pieces(chess.QUEEN, chess.BLACK):
            result -= eg * (7 - _chebyshev(q_sq, wk)) * 8

        for r_sq in board.pieces(chess.ROOK, chess.BLACK):
            rf, rr = chess.square_file(r_sq), chess.square_rank(r_sq)
            if rf == wkf or rr == wkr:
                result -= eg * 50
            result -= eg * (14 - _manhattan(r_sq, wk)) * 4

        for pt in (chess.BISHOP, chess.KNIGHT):
            for sq in board.pieces(pt, chess.BLACK):
                result -= eg * (7 - _chebyshev(sq, wk)) * 3
    ...
    if material_score > 300:
        result += eg * (14 - king_dist) * 5
        result += eg * get_corner_dist(bkf, bkr) * 10
    elif material_score < -300:
        result -= eg * (14 - king_dist) * 5
        result -= eg * get_corner_dist(wkf, wkr) * 10
...
def _king_attack_zone(board: chess.Board, eg: float) -> float:
    ...
    for defender_color in (chess.WHITE, chess.BLACK):
        ...
        if attacker_count >= 3:
            danger = attack_units * attacker_count * 1.3
        elif attacker_count == 2:
            danger = attack_units * 1.8
        else:
            danger = attack_units * 0.7
        score -= defender_sign * danger * mg_weight * 2.0
    return score
...
def _space_advantage(board: chess.Board, eg: float) -> float:
    ...
    return (w_space - b_space) * 1.8 * mg_weight
...
def _pin_strength(board: chess.Board) -> float:
    score = 0.0
    for sq in chess.SQUARES:
        piece = board.piece_at(sq)
        if piece is None or piece.piece_type == chess.KING or piece.piece_type == chess.PAWN:
            continue
...
def _piece_coordination(board: chess.Board, eg: float) -> float:
    if eg > 0.75:
        return 0.0
    score = 0.0
    for color in (chess.WHITE, chess.BLACK):
...
def _back_rank_threats(board: chess.Board) -> float:
    score = 0.0
    for color in (chess.WHITE, chess.BLACK):
        sign = 1 if color == chess.WHITE else -1
        enemy = not color
        ...
        for sq in board.pieces(chess.ROOK, color):
            if chess.square_rank(sq) == back_rank:
                for f in range(8):
                    target_sq = chess.square(f, back_rank)
                    if board.piece_at(target_sq) is None:
                        continue
...
def _advanced_pawns(board: chess.Board, eg: float) -> float:
    if eg > 0.85:
        return 0.0
    score = 0.0
    for color in (chess.WHITE, chess.BLACK):
        ...
        for sq in board.pieces(chess.PAWN, color):
            ...
            if color == chess.WHITE and rank >= 5:
                bonus = (rank - 4) * (rank - 4) * 8
                score += sign * bonus
            elif color == chess.BLACK and rank <= 2:
                bonus = (3 - rank) * (3 - rank) * 8
                score += sign * bonus
    return score

def _evaluate_draw_avoidance(board: chess.Board) -> float:
    ...
    if board.is_stalemate():
        ...
        if perspective_eval > 100:
            return -99000 if _ROOT_COLOR == chess.WHITE else 99000
        elif perspective_eval < -100:
            return 99000 if _ROOT_COLOR == chess.WHITE else -99000
    ...
    if board.is_repetition(3):
        current_eval = evaluate(board)
        perspective_eval = current_eval if _ROOT_COLOR == chess.WHITE else -current_eval
        if perspective_eval > 150:
            return -99000 if _ROOT_COLOR == chess.WHITE else 99000
        elif perspective_eval < -150:
            return 99000 if _ROOT_COLOR == chess.WHITE else -99000
        return -90 if _ROOT_COLOR == chess.WHITE else 90
    ...
    if board.is_fifty_moves():
        ...
        if perspective_eval > 150:
            ...
            return 99000 if _ROOT_COLOR == chess.WHITE else -99000
        return -80 if _ROOT_COLOR == chess.WHITE else 80
...
def evaluate(board: chess.Board) -> float:
    draw_avoid = _evaluate_draw_avoidance(board)
    if draw_avoid != 0.0:
        return draw_avoid
    ...
    score += _pin_strength(board)
    score += _piece_coordination(board, eg)
    score += _back_rank_threats(board)
    score += _advanced_pawns(board, eg)
    
    material_score = _material_and_pst(board, eg)
    score += _king_proximity_mop_up(board, eg, material_score)
    ...
    if board.is_check():
        score += (15 if board.turn == chess.BLACK else -15)
...
_FUTILITY_MARGINS = {1: 150, 2: 350, 3: 600}
_LMP_CUTOFFS      = {1: 10, 2: 15, 3: 25}
_RFP_MARGIN       = 150
...
def minimax(board: chess.Board, depth: int,
    ...
    if (not in_check and ply > 0 and depth <= 7 and eg < 0.90
            and abs(beta) < 90_000):
    ...
    if tt_move is None and depth >= 5 and null_ok:
        depth -= 1
...
def get_next_move(board: chess.Board,
                  color: chess.Color,
                  depth: int = TARGET_DEPTH) -> chess.Move:
