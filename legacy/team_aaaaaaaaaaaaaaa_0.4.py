...
# Global search state
# ------------------------------------------------------------------------------
_TT      = {}                      # transposition table: key -> (depth,flag,score,move)
_KILLERS = [[None, None] for _ in range(128)]
_HISTORY = {}                       # (from_sq, to_sq) -> int
_COUNTERMOVE = {}                   # (prev_from, prev_to) -> move

TT_EXACT, TT_LOWER, TT_UPPER = 0, 1, 2
TARGET_DEPTH = 10                   # base depth (increased when time permits)
CONTEMPT = 60                       # draw avoidance (centipawns)
...
def _endgame_factor(board: chess.Board) -> float:
    """0.0 = opening/middlegame, 1.0 = pure endgame."""
    phase = 0
...
def _material_and_pst(board: chess.Board, eg: float) -> float:
    """Material + interpolated piece‑square value for every piece."""
    score = 0.0
...
def _pawn_structure(board: chess.Board) -> float:
    ...
    for color in (chess.WHITE, chess.BLACK):
        ...
        for sq in pawns:
            ...
            # backward (unsupported, not passed)
...
def _king_attack_zone(board: chess.Board, eg: float) -> float:
    ...
    mg_weight = 1.0 - eg
    score = 0.0
    KA_WEIGHTS = {chess.KNIGHT: 2, chess.BISHOP: 2, chess.ROOK: 3, chess.QUEEN: 5}
    ...
    for defender_color in (chess.WHITE, chess.BLACK):
        ...
        for df in (-1, 0, 1):
            for dr in (-1, 0, 1):
                ...
                if 0 <= nf <= 7 and 0 <= nr <= 7:
                    zone_sqs.append(chess.square(nf, nr))
        ...
        for sq in chess.SQUARES:
            ...
            if piece is None or piece.color != attacker_color:
                continue
            w = KA_WEIGHTS.get(piece.piece_type, 0)
            if w == 0:
                continue
...
def _space_advantage(board: chess.Board, eg: float) -> float:
    if eg > 0.65:
        return 0.0
    mg_weight = 1.0 - eg
    WHITE_SPACE = chess.SquareSet([
        chess.C2, chess.D2, chess.E2, chess.F2,
        chess.C3, chess.D3, chess.E3, chess.F3,
        chess.C4, chess.D4, chess.E4, chess.F4,
    ])
    BLACK_SPACE = chess.SquareSet([
    ...
    for sq in chess.SQUARES:
        ...
        if piece.color == chess.WHITE:
            w_space += len(atk & BLACK_SPACE)
        else:
            b_space += len(atk & WHITE_SPACE)
...
def _trapped_pieces(board: chess.Board) -> float:
    ...
    for sq in chess.SQUARES:
        ...
        if piece.piece_type == chess.BISHOP and mob <= 2:
            score += sign * -30
        elif piece.piece_type == chess.ROOK and mob <= 3:
            score += sign * -25
    return score
...
def _rule_of_square(board: chess.Board, eg: float) -> float:
    ...
    for color in (chess.WHITE, chess.BLACK):
        ...
        if ek is None:
            continue
        # if enemy has any pieces besides king, rule of square doesn't apply
        if any(len(board.pieces(pt, enemy)) > 0 for pt in (chess.QUEEN, chess.ROOK, chess.BISHOP, chess.KNIGHT)):
            continue
        for sq in board.pieces(chess.PAWN, color):
            f = chess.square_file(sq)
            rank = chess.square_rank(sq)
...
def _king_activity_endgame(board: chess.Board, eg: float) -> float:
    ...
    for color in (chess.WHITE, chess.BLACK):
        ...
        score += sign * (7.0 - center_dist) * 5.0 * eg
    return score
...
def _king_opposition(board: chess.Board, eg: float) -> float:
    ...
    if wkf == bkf and abs(wkr - bkr) == 2:
        return (30.0 * eg) if board.turn == chess.BLACK else -30.0 * eg
    if wkr == bkr and abs(wkf - bkf) == 2:
        return (30.0 * eg) if board.turn == chess.BLACK else -30.0 * eg
    return 0.0
...
def _king_proximity_mop_up(board: chess.Board, eg: float, material_score: float) -> float:
    ...
    result = 0.0
    if material_score > 200:          # White is winning
        bkf, bkr = chess.square_file(bk), chess.square_rank(bk)
    ...
    elif material_score < -200:       # Black is winning
        wkf, wkr = chess.square_file(wk), chess.square_rank(wk)
    ...
    if material_score > 300:
        result += eg * ((14 - king_dist) * 4 + corner_dist(bkf, bkr) * 8)
    elif material_score < -300:
        result -= eg * ((14 - king_dist) * 4 + corner_dist(wkf, wkr) * 8)

    return result


def _contempt_score() -> float:
    """Return a negative value for the side to move if the game is drawn."""
...
def evaluate(board: chess.Board) -> float:
    """Return the evaluation from White's perspective (positive = good for White)."""
    # Terminal checkmate
    if board.is_checkmate():
        return -99_999 if board.turn == chess.WHITE else 99_999

    # Draws – use contempt to avoid them unless behind
...
def _qsearch(board: chess.Board, alpha: float, beta: float, maximizing: bool) -> float:
    ...
    if maximizing:
        if stand_pat >= beta:
            return beta
        if stand_pat + _DELTA_PRUNE < alpha:
            return alpha
        if stand_pat > alpha:
            alpha = stand_pat
        ...
        if stand_pat <= alpha:
            return alpha
        if stand_pat - _DELTA_PRUNE > beta:
            return beta
        if stand_pat < beta:
            beta = stand_pat
...
# ------------------------------------------------------------------------------
# Main search (minimax with alpha‑beta and all pruning techniques)
# ------------------------------------------------------------------------------
def minimax(board: chess.Board, depth: int,
    ...
    if (not in_check and ply > 0 and depth <= 6 and eg < 0.90 and
            abs(beta) < 90_000):
    ...
    # Internal iterative deepening (if no TT move)
    ...
    if maximizing:
        for move_idx, move in enumerate(moves):
            ...
            if (is_quiet and not in_check and depth in _FUTILITY_MARGINS and
                    move_idx > 0 and abs(alpha) < 90_000):
            ...
            # Late move pruning (depth 1-3, stop after N quiet moves)
            if (is_quiet and not in_check and depth in _LMP_CUTOFFS and
                    quiet_count >= _LMP_CUTOFFS[depth] and abs(alpha) < 90_000):
                continue
            ...
            # Late move reduction
            reduce = 0
            if (move_idx >= 2 and depth >= 3 and is_quiet and
                    not in_check and not gives_check):
                reduce = max(1, int(_log(max(depth, 1)) * _log(move_idx + 1) / 2.25))
            ...
            if move_idx == 0:
                ...
                score = minimax(board, depth - 1 - reduce, alpha, alpha + 1, False,
                                ply + 1, True, move)
                if score > alpha:
        ...
        flag = (TT_EXACT if orig_alpha < best < orig_beta else
                (TT_LOWER if best >= orig_beta else TT_UPPER))
        ...
        for move_idx, move in enumerate(moves):
            ...
            if (is_quiet and not in_check and depth in _FUTILITY_MARGINS and
                    move_idx > 0 and abs(beta) < 90_000):
                if static_eval is None:
                    static_eval = evaluate(board)
            ...
            if (is_quiet and not in_check and depth in _LMP_CUTOFFS and
                    quiet_count >= _LMP_CUTOFFS[depth] and abs(beta) < 90_000):
                continue
            ...
            if move_idx == 0:
                ...
                score = minimax(board, depth - 1 - reduce, beta - 1, beta, True,
                                ply + 1, True, move)
                if score < beta:
                    score = minimax(board, depth - 1, alpha, beta, True,
                                    ply + 1, True, move)
        ...
        flag = (TT_EXACT if orig_alpha < best < orig_beta else
                (TT_UPPER if best <= orig_alpha else TT_LOWER))
        _tt_store(h, depth, flag, best, best_move)
        return best
...
def get_next_move(board: chess.Board, color: chess.Color, depth: int = 3) -> chess.Move:
    ...
    # Reset per‑search data (keep TT across moves)
    _KILLERS = [[None, None] for _ in range(128)]
...
if __name__ == '__main__':
    ...
    for d in (3, 5, TARGET_DEPTH):
        ...
        print(f"depth={d:2d}: {b.san(move):6s}  ({elapsed:.2f}s)")