"""
team_astar.py — Hybrid NN + handcrafted evaluation with full search infrastructure.

Optimizations:
  • Cython board_to_vector via bitboard scanning (30-50× faster vectorization)
  • Pre-allocated tensor buffer for NN eval (no per-call allocation)
  • CPU inference for NN (GPU round-trip is slower than CPU matmul for 1 position)
  • Cython HCE with flattened PST arrays (if available)
"""
...
# ── NN model import ───────────────────────────────────────────────────────────
from madeinorbit.train_nn import ChessEvaluator, SCORE_CLAMP

# Force CPU for inference — GPU round-trip latency destroys single-position perf
_device = torch.device("cpu")
_model  = ChessEvaluator()
_model_path = Path(__file__).resolve().parent.parent / "madeinorbit" / "model.pt"
...
_PASSED  = [0, 10, 20, 35, 60, 90, 130, 0]
...
# ── NN evaluation — optimized ─────────────────────────────────────────────────
def _nn_eval(board):
    # Write into pre-allocated buffer — no torch.tensor() allocation per call
    vec = board_to_vector(board)  # Cython: ~50× faster than Python
...
def evaluate(board):
    ...
    if (board.is_stalemate() or board.is_insufficient_material() or
        ...
        return 0.0
    return (1.0 - NN_WEIGHT) * _hce(board) + NN_WEIGHT * _nn_eval(board)
...
if __name__ == '__main__':
    ...
    if move is not None:
        print(f"[team_astar] Opening move: {b.san(move)}")
